...
def _migrate_existing_schema(cur):
    ...
    # One-time 7-day Creator trial marker (PRD section 2.4): a trial can
    # never be re-granted after expiry/restart because the marker column
    # persists. See services/plan_service.start_trial.
    if not _column_exists(cur, "users", "trial_granted_at"):
        cur.execute("ALTER TABLE users ADD COLUMN trial_granted_at TIMESTAMP")
    ...
    # user_telegram_sessions grew ownership columns after older
    # databases created the table (see init_schema) - add them in the
    # same migration pass, guarded so this never runs before the table
    # itself exists (PRD 46: no migration may alter a missing table).
    cur.execute("SELECT 1 FROM sqlite_master WHERE type='table' AND name='user_telegram_sessions'")
    if cur.fetchone():
        ...
        if not _column_exists(cur, "user_telegram_sessions", "external_username"):
            cur.execute("ALTER TABLE user_telegram_sessions ADD COLUMN external_username TEXT")
        if not _column_exists(cur, "user_telegram_sessions", "needs_reconnect"):
            cur.execute("ALTER TABLE user_telegram_sessions ADD COLUMN needs_reconnect INTEGER NOT NULL DEFAULT 0")
        if not _column_exists(cur, "user_telegram_sessions", "last_seen_at"):
            cur.execute("ALTER TABLE user_telegram_sessions ADD COLUMN last_seen_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP")
        cur.execute("""
            CREATE UNIQUE INDEX IF NOT EXISTS idx_uts_one_active_owner
            ON user_telegram_sessions(external_telegram_id)
            WHERE external_telegram_id IS NOT NULL AND status='connected'
        """)
...
def _migrate_late_tables(cur):
    ...
    # Telegram Stars pricing (PRD section 26): read by
    # plan_service.get_plan_stars_price and payment flows. Older
    # databases created plan_configs before this column existed - a
    # missing column crashes get_entitlements() (row["stars_monthly_price"]
    # IndexError), so existing databases MUST be migrated here (the
    # table already exists by the time _migrate_late_tables runs).
    if not _column_exists(cur, "plan_configs", "stars_monthly_price"):
        cur.execute("ALTER TABLE plan_configs ADD COLUMN stars_monthly_price INTEGER NOT NULL DEFAULT 0")
        cur.execute("UPDATE plan_configs SET stars_monthly_price=99 WHERE plan_name='BEGINNER'")
        cur.execute("UPDATE plan_configs SET stars_monthly_price=249 WHERE plan_name='PRO'")
        cur.execute("UPDATE plan_configs SET stars_monthly_price=499 WHERE plan_name='CREATOR'")
...
def init_db():
    ...
    #
    # telegram_id          -> ChannelFlow user id (the SaaS account)
    # external_telegram_id -> the REAL external Telegram account id
    #                         returned by Telethon get_me() (PRD 6.1/6.2)
    # status='connected' rows enforce "ONE active ChannelFlow owner per
    # external Telegram account" via the partial unique index
    # idx_uts_one_active_owner below (PRD 6.3/6.4/6.5).
    ...
    # Ownership uniqueness backstop (PRD 6.4): exactly one ACTIVE owner
    # per external Telegram account, enforced by the database so a race
    # between two /connect flows can never create two owners. A partial
    # index only over connected rows lets a user reclaim/disconnect and
    # another user connect the same account afterwards (PRD 6.7).
    #
    # On a FRESH database the column exists right now; on an EXISTING
    # database the column is only added later in init_db (the late
    # migration pass), so creating the index here is guarded on the
    # column being present. init_db re-creates the index unconditionally
    # at the end, after migrations, so no database misses it.
    if _column_exists(cur, "user_telegram_sessions", "external_telegram_id"):
        cur.execute("""
            CREATE UNIQUE INDEX IF NOT EXISTS idx_uts_one_active_owner
            ON user_telegram_sessions(external_telegram_id)
            WHERE external_telegram_id IS NOT NULL AND status='connected'
    ...
    # Backfill note: legacy rows stored the ChannelFlow user id in a
    # column named telegram_id with no external-id concept; they are
    # migrated by _migrate_existing_schema (adds the columns later in
    # init_db once every table exists). Nothing extra to backfill here.
    #
    ...
    if cur.fetchone()[0] == 0:
        ...
        for plan in default_plans:
            cur.execute("""
                INSERT INTO plan_configs(plan_name, display_name, monthly_price_inr, crypto_monthly_price_usd, stars_monthly_price, max_projects, max_sources_per_project,
            ...
            """, (
                ...
                plan["crypto_monthly_price_usd"], plan["stars_monthly_price"],
    ...
    # Re-create the ownership-uniqueness index. On a FRESH database the
    # index already exists (created with user_telegram_sessions); on an
    # EXISTING database _migrate_existing_schema added the ownership
    # columns above and created the index there. Re-running is a cheap
    # no-op either way, and guarantees the constraint exists even for a
    # partially-migrated database that skipped both earlier paths.
    cur.execute(
        "CREATE UNIQUE INDEX IF NOT EXISTS idx_uts_one_active_owner "
        "ON user_telegram_sessions(external_telegram_id) "
        "WHERE external_telegram_id IS NOT NULL AND status='connected'"
