...
# user_id -> time.monotonic() when WAITING_CONNECT_PHONE /
# WAITING_CONNECT_STAGE was last (re)set. Values are timestamps so the
# single connect-state cleanup task in core.user_sessions can expire
# both dicts in lock-step with the pending Telethon clients
# (PRD sections 43/44 - stale-flow cleanup). WAITING_CONNECT_PHONE
# values are timestamps directly (handlers only test membership).
