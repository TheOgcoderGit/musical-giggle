...
def approve_payment(request_id, admin_id):
    ...
    cur.execute(
        "UPDATE payment_requests SET status='APPROVED', decided_at=CURRENT_TIMESTAMP, decided_by=? WHERE id=? AND status='SUBMITTED'",
        (admin_id, request_id),
    )
    ...
    if not changed:
        return None  # already decided - duplicate-approval guard
    ...
    if request["purpose"] == "wallet_topup":
        ...
        _activate_plan_purchase(request, actor_label=f"admin:{admin_id}")

    return get_payment_request(request_id)


def reject_payment(request_id, admin_id, reason=None):
    """Rejects a submitted request. `reason` is optional, shown to the
    user in the rejection notice and stored on the request row."""

    conn = get_connection()
    cur = conn.cursor()

    if not _column_exists(cur, "payment_requests", "admin_notes"):
        cur.execute("ALTER TABLE payment_requests ADD COLUMN admin_notes TEXT")

    cur.execute(
        "UPDATE payment_requests SET status='REJECTED', decided_at=CURRENT_TIMESTAMP, decided_by=?, admin_notes=? WHERE id=? AND status='SUBMITTED'",
...
def expire_stale_requests():
    """Auto-expires stale UPI/screenshot requests past STALE_HOURS, plus
    any crypto requests past their configured expiry. Called from the
    subscription scheduler loop. Returns expired rows for notification."""

    conn = get_connection()
    cur = conn.cursor()

    # Legacy long-stale requests (UPI screenshot flow)
    cur.execute(
        """
    ...
    # Crypto invoices past their short expiry window
    cur.execute(
        """
        SELECT * FROM payment_requests
        WHERE method = 'crypto'
          AND status IN ('PENDING_PAYMENT', 'DETECTING', 'CONFIRMING')
          AND expires_at IS NOT NULL
          AND expires_at <= ?
        """,
        (datetime.now(timezone.utc).isoformat(),),
    )
    crypto_expired = list(cur.fetchall())
