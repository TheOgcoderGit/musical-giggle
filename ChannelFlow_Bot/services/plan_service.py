...
def start_trial(telegram_id):
    """Grants the one-time 7-day CREATOR trial (PRD section 2.4).

    Idempotent + persistent: the marker column users.trial_granted_at
    is set on first grant and never cleared, so a trial can never be
    re-granted by restarts, repeated /start, or expiry. Only fires for
    a user who has never had ANY trial (marker NULL) and is still on
    the FREE baseline plan.

    Returns True when this call granted the trial, False otherwise."""

    conn = get_connection()
    cur = conn.cursor()

    cur.execute(
        "SELECT plan, plan_expiry, trial_granted_at FROM users WHERE telegram_id=?",
        (telegram_id,),
    )
    row = cur.fetchone()
    ...
    # Already granted a trial once - never again, even after expiry or
    # a bot restart.
    if row["trial_granted_at"] is not None:
        conn.close()
        return False

    # A user who already moved off FREE (e.g. admin-assigned plan or a
    # paid plan from before this marker existed) never gets a trial
    # retroactively - that would be an unlimited-trial loophole.
    if row["plan"] != "FREE" or row["plan_expiry"] is not None:
        conn.close()
        return False
    ...
    cur.execute(
        "UPDATE users SET plan=?, plan_expiry=?, trial_granted_at=CURRENT_TIMESTAMP "
        "WHERE telegram_id=? AND trial_granted_at IS NULL",
