...
Rewrites/transforms content via OpenRouter (https://openrouter.ai).
...
async def rewrite_content(
    ...
    if not OPENROUTER_API_KEY:
        return AIResult(success=False, text=text, error="AI provider not configured")
    ...
    # Model chain: override > configured default > free fallbacks
    models_to_try = []
    primary = settings.get("model_override") or AI_MODEL
        ...
        for retry in range(MAX_RETRIES + 1):
            ...
            try:
                ...
                if resp.status_code == 429:
                    # Rate limited by provider - wait briefly then retry
                ...
                if resp.status_code >= 400:
                    ...
                    try:
                        ...
                        pass
                    logger.warning(
                        "AI request failed (model=%s status=%s): %s",
                        model, resp.status_code, error_detail
                    )
                ...
                rewritten = choices[0].get("message", {}).get("content")
                if not rewritten:
                    last_error = "Empty completion"
                    break

                # Validate + enforce preservation rules
                final_text = _validate_output(text, rewritten.strip(), settings)
                ...
                logger.exception("AI request exception (model=%s)", model)
