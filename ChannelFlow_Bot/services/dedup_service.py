...
def cleanup_expired_claims():
    """Deletes claims stuck in 'claimed' longer than CLAIM_TTL_SECONDS -
    crashed workers shouldn't block retries forever.
    ...
    cur.execute(
        ...
        WHERE status='claimed'
          AND created_at <= datetime('now', '-{CLAIM_TTL_SECONDS} seconds')
        """
    )
    ...
    if deleted:
        logger.info("Cleaned up %s expired dedup claims", deleted)
