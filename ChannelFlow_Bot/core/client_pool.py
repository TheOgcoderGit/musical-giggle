    ...
    async with _engines_lock:
        ...
        try:
            ...
            if not await client.is_user_authorized():
                ...
                # PRD 7.3: an invalid session must not crash the bot -
                # surface it as "needs reconnection" so the UI can show a
                # Reconnect action instead of silently staying dead.
                user_sessions.mark_needs_reconnect(owner_id, True)
                return

            # Stored session recovered - clear any stale reconnect flag.
            user_sessions.mark_needs_reconnect(owner_id, False)
            ...
            try:
                ...
                pass
            # A connection failure is not (yet) proof the session itself
            # is invalid - network/FloodWait can look identical. Only
            # mark needs_reconnect on a definitive auth failure above.
            return
